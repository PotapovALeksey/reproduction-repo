...
import {UserMainHeaderSelector} from "../../src/screens/main/user-main/user-main-header/user-main-header.selector";
...
describe('Login User screens', () => {
  ...
  it('User-card is decreased after swiping-up', async () => {
    ...
    await waitFor(element(by.id(UserCardSelector.Graphic))).toBeVisible();
    await element(by.id(UserMainHeaderSelector.ControlButtons)).swipe('up');
    await waitFor(element(by.id(UserCardSelector.Graphic))).toBeNotVisible();
  });
});
